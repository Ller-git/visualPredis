const Chash = {
	template: `
		<div class="panel panel-default" style="margin: 30px;border-radius: 0px;box-shadow: 0 2px 12px 0 rgba(0,0,0,.1);">
		    ...
		    <div class="panel-body">
				...
				<p>&nbsp;</p>
				<template v-for="item in form.hash">
		        <div class="form-group row">
				    <label for="firstname" class="col-sm-1 col-sm-offset-1 control-label"><span style="color: #f56c6c;">*</span>Field</label>
		        	<div class="col-sm-9">
						<input placeholder="field" v-model="item.field" style="width: 150px; display: inline-block;" type="text" class="form-control">
	...
	methods: {
		// 新增字段
		addHash: function () {
			this.form.hash.push({field: '', value: ''});
		},
		// 提交创建字段
		submitHash: function() {
			this.$redis.cHash(this.form).then((result)=>{
		...
		delHash: function(item) {
		    ...
		    var index = this.form.hash.indexOf(item);
		    if (index != -1) {
		    	this.form.hash.splice(index, 1);
		    }
		},
