const Elist = {
		...
		<div class="panel panel-default" style="margin: 30px;box-shadow: 0 2px 12px 0 rgba(0,0,0,.1);">
			    ...
			    <div class="modal-dialog">
			        <div class="modal-content">
			            <div class="modal-header">
			                ...
			                <h4 class="modal-title" id="myModalLabel">Update&nbsp;{{ update_form.index }}</h4>
			            </div>
			            ...
			            <div class="modal-footer" style="border: none;">
			                ...
			                <button @click="updateList" type="button" class="btn btn-primary">Submit</button>
			            </div>
			        </div><!-- /.modal-content -->
			    </div><!-- /.modal -->
			</div>
		    <div class="panel-heading" style="background-color: #fff;">
		        <h3 class="panel-title">
		        	...
		        	<button @click="delList()" type="button" class="pull-right btn btn-sm btn-danger glyphicon glyphicon-trash"></button>
		        </h3>
		    </div>
		    <div class="panel-body">
				...
				<template v-for="(item, index) in insert_form.list">
		        <div class="form-group row">
		        ...
		        <div class="form-group row">
				    <div class="col-sm-offset-2 col-sm-9">
				    	<button @click="insertList('left')" type="button" class="btn btn-primary">Left</button>
				    	<button @click="insertList('right')" type="button" class="btn btn-primary">Right</button>
				    </div>
				</div>
				<hr />
				<div class="form-group row">
				    <label for="firstname" class="col-sm-1 col-sm-offset-1 control-label">Indexs</label>
				    <div class="col-sm-9">
				    	<!-- 分页 -->
				    	<div v-if="key_info.pages != 1" class="input-group" style="width: 360px;">
				            <span class="input-group-addon" @click="requestList('prev')">prev</span>
				            <span class="input-group-addon" @click="requestList('next')">next</span>
				            <input type="text" v-model="key_info.page" class="form-control text-center">
				            <span @click="requestList('jump')" class="input-group-addon">jump</span>
				        </div>
				        <!-- /分页 -->
				    	<table class="table">
							...
							<template v-for="(item, index) in key_info.val">
								<tr>
									<td>{{ index }}</td>
									<td>{{ item }}</td>
									<td><button @click="updatingList(index)" class="btn btn-sm btn-primary glyphicon glyphicon-pencil"></button>
									<button @click="delList(index)" class="btn btn-sm btn-danger glyphicon glyphicon-trash"></button></td>
								</tr>			
							</template>
							</tbody>
						</table>
	...
	methods: {
		...
		// 新增列表
		insertList: function(methods) {
			this.insert_form.methods = methods;
			this.$redis.cList(this.insert_form).then((result)=>{
				// 返回结果之后清空输入内容，并且更新数据
				this.requestList();
			})
		},
		// 更新正在收集信息
		updatingList: function(index) {
			// 1. 获取将要修改的哈希字段
		...
		// 更新
		updateList: function() {
			this.$redis.eList(this.update_form).then((result)=>{
		...
		delList: function(index = '') {
			...
			if (index === '') {
				...
				this.$redis.dList({key: this.key, index: index}).then((result)=>{
					this.requestList();
				})
			}
		},
